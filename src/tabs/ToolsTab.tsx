...
export const ToolsTab: React.FC<Props> = ({ card }) => {
  ...
  } catch { /* ignore localStorage read */ }
  }, []);
  ...
  } catch { /* ignore localStorage write */ }
...
const ReversePlateCalculator: React.FC<{ hideTitle?: boolean }> = ({ hideTitle }) => {
  ...
  useEffect(()=>{
  try { localStorage.setItem(REVERSE_KEY, JSON.stringify({ u:unit, b:bar, c:custom, counts })); } catch { /* ignore quota */ }
